...
// Run a curve to completion, recording one sample per emitted setpoint. v_in/v_out
// are given in counts/s for readability and crossed to wire units via the library's
// curve_cps_to_wire (the same crossing the host/firmware use); speed is read back
// in counts/s via curve_speed_cps (the interpolator stores it tick-native).
std::vector<Sample> run(curve_state_t &st, const curve_limits_t &lim,
    ...
    curve_interp_begin(&st, &lim, p0x, p0y, p1x, p1y, p2x, p2y, p3x, p3y,
                       curve_cps_to_wire(v_in, lim.dt_tick_us),
                       curve_cps_to_wire(v_out, lim.dt_tick_us));
    ...
    while (going && n < max_steps) {
        ...
        out.push_back({(double)x, (double)y, curve_speed_cps(&st, lim.dt_tick_us)});
        n++;
    }
